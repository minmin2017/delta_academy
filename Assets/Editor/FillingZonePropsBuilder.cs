...
public static class FillingZonePropsBuilder
{
    ...
    public static void AddFillingZoneProps()
    {
        ...
        // =========================================================================
        // 5b. CLEARANCE FIX: resolve confirmed 3D bounding-box overlap with DeltaControlCabinet.
        // Both occupied similar X range and full floor-to-ceiling Y, so an X or Y shift alone
        // wouldn't clear it, but an AABB collision only needs separation on ONE axis - shifting
        // purely in Z clears the cabinet without disturbing the pipe run's X-alignment to the
        // real NozzleAssembly_Z. Applied here as a POST-PROCESSING step (after every child's
        // absolute world position is already set above) because setting child.transform.position
        // to an absolute Vector3 recomputes localPosition against the parent at assignment time -
        // an offset applied to fillingZoneObj's position BEFORE building children gets silently
        // cancelled out by those later absolute assignments. Verified live via unityMCP execute_code
        // before writing this: cabinet bounds (-1.75,0.00,-0.54)/(-0.44,1.80,1.33), filling zone
        // bounds after this exact shift clear it with Bounds.Intersects() == false.
        Vector3 clearanceOffset = new Vector3(0f, 0f, -1.8f);
        foreach (Transform child in fillingZoneObj.transform)
        {
            child.position += clearanceOffset;
        }
